    ...
    async userConfirm({ commit }, auth) {
      await onAuthStateChanged(auth, (user) => {
        if (user) {
          ...
          // const uid = user.uid;
          //ログイン状態をtrueにする
          commit("userConfirm", { judge: true, id: user.uid })
          //ボート情報を読み込むためにID付与(boatId = userId)
          commit("createBoatId", user.uid)
          console.log("vuex:userConfirm:ログインしました"),
          console.log("vuex:userConfirm:ユーザーID:" + user.uid),
          console.log("vuex:userConfirm:ボートID:" + user.uid)
        } else {
          //ログイン状態をfalaseにする
          console.log("vuex:userConfirm:ログアウトしました")
          commit("userConfirm", true)
        }
      });
    },
